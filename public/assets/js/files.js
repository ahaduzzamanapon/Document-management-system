        ...
        success: function (data) {
            ...
            if (Object.keys(folder_data).length > 0) {
                Object.keys(folder_data).forEach(function (key) {
                    ...
                    var newItem = ` <section draggable="true" data-id="${item.id}" data-name="${item.name}" data-type="folder"  ondblclick="fetch_data(${item.id})"  class="folderclick drag_box cursor-pointer rounded-md w-[95%]  lg:w-[30.9%]  xl:w-[23%]   bg-[#EBFBEC] py-3  placeholder:flex items-center justify-between gap-1" >
            ...
            if (Object.keys(fileData).length > 0) {
                Object.keys(fileData).forEach(function (key) {
                    ...
                    var newItem = `<section draggable="true" data-id="${file_item.id}" data-name="${file_item.title}" data-type="file"  class="drag_box w-[43%] lg:w-[21.2%] xl:w-[17%] px-3 pt-3 bg-[#F4F4F4] rounded-md border border-[#cfcfcf] file_link" ondblclick="displayFileContents('${file_item.id}');">
